...
class SemanticKittiDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', balance_classes=False, seqential_batch = False):
        ...
        if self.set == 'training':
            self.sequences = ['{:02d}'.format(i) for i in range(1,11)]
        elif self.set == 'validation':
            self.sequences = ['00']# self.sequences = ['{:02d}'.format(i) for i in range(11) if i == 8]
        elif self.set == 'test':
            self.sequences = ['00']# self.sequences = ['{:02d}'.format(i) for i in range(11, 22)]
        else:
        ...
        while True:
            ...
            while num_merged < self.config.n_frames and f_ind - f_inc >= 0:
                ...
                if self.set == 'test':
                    ...
                    # frame_labels = frame_labels[p_idx]
...
class SemanticKittiStreamDataset(PointCloudStreamDataset):
    """Class to handle SemanticKitti dataset."""

    def __init__(self, config, set='training', balance_classes=False, seqential_batch = False):
        PointCloudStreamDataset.__init__(self, 'SemanticKittiStream')
        ...
        # Initialize frame potentials
        self.potentials = torch.from_numpy(np.random.rand(config.n_test_frames) * 0.1 + 0.1)
        if seqential_batch:
            self.potentials = torch.from_numpy(np.zeros(config.n_test_frames))
        self.potentials.share_memory_()
    ...
    def getitem(self, batch_i):
            ...
            while num_merged < self.config.n_frames and f_ind - f_inc >= 0:
                ...
                if self.set in ['validation', 'test'] and self.config.n_test_frames > 1 and f_inc > 0:
                    ...
                    if self.config.sampling == 'objectness':
                        ...
                        print("filename of current frame ", filename)
                        file_path = join(test_path, filename)
                        center_pred = None
                        counter = 0
                        while center_pred is None:
                ...
                num_merged += 1
                f_inc += 1
                print("number of frames merged: ", num_merged)
        ...
        # Add scale and rotation for testing
        input_list += [scales, rots, frame_inds, frame_centers, centers, times, ins_labels.astype(np.int64), \
                        r_inds_list, r_mask_list, f_inc_r_inds_list, f_inc_r_mask_list, val_labels_list, val_center_label_list]
    ...
    def parse_poses(self, filename, calibration):
        ...
        values = [float(v) for v in file.readline().strip().split()]
    ...
    def __iter__(self):
        ...
        if self.dataset.balance_classes:
            ...
            for i, c in enumerate(self.dataset.label_values):
                if c not in self.dataset.ignored_labels:  #[4,6,9,17]: 
                    ...
                    # print(c, self.dataset.ignored_labels)
                    used_classes = 4 # self.dataset.num_classes - len(self.dataset.ignored_labels)
                    class_n = num_centers // used_classes + 1
                    # print(class_potentials.shape[0], class_n)
                    if class_n < class_potentials.shape[0]:
                        _, class_indices = torch.topk(class_potentials, class_n, largest=False)
                        # print(class_potentials.shape[0])
                    else:
            ...
            # print(gen_indices.shape[0], num_centers)

            # Shuffle generated indices
            rand_order = torch.randperm(gen_indices.shape[0])[:num_centers]
            # print(rand_order.shape[0])
            if num_centers > gen_indices.shape[0]:
                print('num-centers = ', num_centers, 'gen-indices = ', gen_indices.shape[0])
                extra_order = torch.randperm(num_centers-gen_indices.shape[0])
                # print(rand_order.size(),num_centers,gen_indices.shape[0])
                # rand_order = torch.cat((rand_order, extra_order), dim=0) #rand_order[:gen_indices.shape[0]]#
                rand_order = torch.randint(gen_indices.shape[0],(num_centers,))
            ...
            if  num_centers < self.dataset.potentials.shape[0]:
                #gen_indices = torch.randperm(self.dataset.potentials.shape[0])[:num_centers]
                _, gen_indices = torch.topk(self.dataset.potentials, num_centers, largest=False, sorted=True)
                # print('num-centers = ', num_centers, 'gen-indices = ', gen_indices.shape[0], 'N =',self.N)
