    ...
    def augmentation_transform(self, points, normals=None, verbose=False):
        ...
        if points.shape[1] == 3:
            if self.config.augment_rotation == 'vertical':
                ...
                # changed by wangkang, set theta = 0 so it wrong rotate.
                theta = 0
        ...
        # changed by wangkang, set scale = 1 so it won't scale
        scale = 1
        ...
        # changed by wangkang, set noise = 0 so it won't add noise
        noise = 0
        ##################
        # Apply transforms
        ##################
...
class PointCloudStreamDataset(IterableDataset):
    """Parent class for Point Cloud Datasets."""
