...
class StreamStitchTracklet():
    
    def __init__(self) -> None:
        pass

    def associate_instances(self, previous_instances, current_instances, overlaps,  pose, association_weights):
        pose = pose.cpu().float()
    ...
    def associate_instances_overlapping_frames(self, previous_ins_label, current_ins_label):
        ...
        previous_instance_ids = [x for i,x in enumerate(previous_instance_ids) if c_p[i] > 5] #25 
        current_instance_ids = [x for i, x in enumerate(current_instance_ids) if c_c[i] > 10] #50
    ...
    def main(self, FLAGS):
        self.data_cfg = 'streams/semantic-kitti.yaml'
        self.DATA = yaml.safe_load(open(self.data_cfg, 'r'))
        self.split = 'test'
        self.dataset = 'streams/'

        self.prediction_dir =  FLAGS.predictions
        if self.split == 'valid':
            prediction_path = '{}/val_probs'.format(self.prediction_dir)
        else:
            prediction_path = '{}/probs'.format(self.prediction_dir)
        self.n_test_frames = FLAGS.n_test_frames

        self.association_weights = [FLAGS.iou_3d, FLAGS.iou_2d, FLAGS.center, FLAGS.feature]

        self.association_names = ['3d', '2d', 'cen', 'fet']
        self.assoc_saving = [asc_type for idx, asc_type in enumerate(self.association_names) if
                        self.association_weights[idx] > 0]
        self.assoc_saving.append(str(self.n_test_frames))
        self.assoc_saving = '_'.join(self.assoc_saving)

        self.save_path = '{}/stitch'.format(self.prediction_dir)+self.assoc_saving
        # if not os.path.exists(self.save_path):
        #     os.makedirs(self.save_path)

        learning_map_doc = self.DATA['learning_map']
        self.inv_learning_map_doc = self.DATA['learning_map_inv']

        self.inv_learning_map = np.zeros((np.max([k for k in self.inv_learning_map_doc.keys()]) + 1), dtype=np.int32)
        for k, v in self.inv_learning_map_doc.items():
            self.inv_learning_map[k] = v

        # get number of interest classes, and the label mappings
        # class
        class_remap = self.DATA["learning_map"]
        class_inv_remap = self.DATA["learning_map_inv"]
        class_ignore = self.DATA["learning_ignore"]
        nr_classes = len(class_inv_remap)
        class_strings = self.DATA["labels"]

        # make lookup table for mapping
        # class
        self.maxkey = max(class_remap.keys())

        # +100 hack making lut bigger just in case there are unknown labels
        self.class_lut = np.zeros((self.maxkey + 100), dtype=np.int32)
        self.class_lut[list(class_remap.keys())] = list(class_remap.values())

        # class
        self.ignore_class = [cl for cl, ignored in class_ignore.items() if ignored]

        print("Ignoring classes: ", self.ignore_class)

        # get test set

        self.poses = []

        self.calib = parse_calibration(os.path.join(self.dataset, "calib.txt"))
        poses_f64 = parse_poses_stream(os.path.join(self.dataset, "poses.txt"), self.calib)
        self.poses.append([pose.astype(np.float32) for pose in poses_f64])

        self.prev_instances = {}
        self.overlap_history = {}
    
    def generate_labels(self, sem_results, ins_results, frame):
        ...
        label_sem_class = sem_results
        # label_inst = np.load(ins_path)
        label_inst = ins_results
        ...
        #         association_costs, associations = self.associate_instances_overlapping_frames(prev_inst_orig* things, prev_inst* things)
        ...
        #         prev_point_path = os.path.join(dataset, "sequences", '{0:02d}'.format(int(sequence)), "velodyne", '{0:06d}.bin'.format(idx-i+frame_offset))
        #         #pose = poses[0][idx-i]
        ...
        if len(self.prev_instances.keys()) > 0:
            #firstly associate overlapping instances
            for (new_id, prev_id) in overlaps.items():
                ins_points = torch.where((label_inst == new_id))
                if not new_id in new_instances or prev_id not in self.prev_instances:
                    continue
                self.overlap_history[new_id] = prev_id#add tracking id
                label_inst[ins_points[0]] = prev_id
                self.prev_instances[prev_id]['bbox_proj'] = new_instances[new_id]['bbox_proj']
                self.prev_instances[prev_id]['mean'] = new_instances[new_id]['mean']
                self.prev_instances[prev_id]['center'] = new_instances[new_id]['center']

                self.prev_instances[prev_id]['life'] += 1
                self.prev_instances[prev_id]['tracker'].update(new_instances[new_id]['kalman_bbox'], prev_id)
                self.prev_instances[prev_id]['kalman_bbox'] = torch.from_numpy(self.prev_instances[prev_id]['tracker'].get_state()).float()
                self.prev_instances[prev_id]['bbox'] = kalman_box_to_eight_point(self.prev_instances[prev_id]['kalman_bbox'])
            ...
            for prev_id, new_id in associations:
                ...
                ins_points = torch.where((label_inst == new_id))
                label_inst[ins_points[0]] = prev_id
                self.overlap_history[new_id] = prev_id
                self.prev_instances[prev_id]['bbox_proj'] = new_instances[new_id]['bbox_proj']
                self.prev_instances[prev_id]['mean'] = new_instances[new_id]['mean']
                self.prev_instances[prev_id]['center'] = new_instances[new_id]['center']
        ...
        for ins_id, instance in new_instances.items():  # add new instances to history
            ...
            if ids[0].shape[0] < 10:
                continue
            self.prev_instances[ins_id] = instance

        # kill instances which are not tracked for a  while
        dont_track_ids = []
        for ins_id in self.prev_instances.keys():
            if self.prev_instances[ins_id]['life'] == 0:
                dont_track_ids.append(ins_id)
            self.prev_instances[ins_id]['life'] -= 1

        for ins_id in dont_track_ids:
            del self.prev_instances[ins_id]
        ...
        inv_sem_labels = self.inv_learning_map[sem_pred]
        new_preds = np.bitwise_or(new_preds,inv_sem_labels)

        # new_preds.tofile('{}/{}/{:02d}/predictions/{:06d}.label'.format(save_path, 'sequences', sequence, idx + frame_offset))
    ...
    def __init__(self):
        ...
        # FLAGS.sequences = [int(FLAGS.predictions.strip().split('/')[-1].split('_')[-1])]
        # FLAGS.sequences = [int(x) for x in FLAGS.sequences.split(',')]
        self.main(self.FLAGS)
