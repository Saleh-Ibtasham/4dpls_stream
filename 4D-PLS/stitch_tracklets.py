...
def main(FLAGS):
    ...
    for sequence in test_sequences:
        ...
        poses_f64 = parse_poses_stream(os.path.join(dataset, "sequences", '{0:02d}'.format(sequence), "poses.txt"), calib)
        poses.append([pose.astype(np.float32) for pose in poses_f64])

    frame_offset = int(np.sort(os.listdir(os.path.join(dataset, "sequences", '{0:02d}'.format(sequence), "velodyne")))[0][:-4])
